...
extension ViewController {
    ...
    private func addTargets() {
        ...
        routeButton.addTarget(self, action: #selector(ResetRoute), for: .touchUpInside)
        resetButton.addTarget(self, action: #selector(route), for: .touchUpInside)
    ...
    private func addDirectional(startCoordinate: CLLocationCoordinate2D, endCoordinate: CLLocationCoordinate2D) {
        ...
        let endPlacemarl = MKPlacemark(coordinate: endCoordinate)
        request.source = MKMapItem(placemark: startPlaceMark)
        request.destination = MKMapItem(placemark: endPlacemarl)
        ...
        directional.calculate { (response, error) in
            if error == nil {
                ...
                self.showAlert(title: "Ошибка", message: "Ошибка серверп", searchTextField: false, buttonTitle: "Ок") { (_) in
                    print(" ")
                    return
                }
            }
